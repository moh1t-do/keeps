            ...
            <div>
                ...
                <div className="sm:flex items-center gap-10 hidden">
                    <ul className="flex gap-10 cursor-pointer items-center">
                        ...
                        {authUser && <li><button onClick={handleSignOut} className="h-8 px-4 py-2 font-bold  uppercase transition-all  text-xs bg-blue-500 text-white shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none">Logout</button></li>}
                    </ul>
                </div>
                {/* mobile  */}
                {isOpen && (
                    <div className="w-screen fixed sm:hidden top left-0 h-full py-4 bg-black">
                        <div className="max-w-6xl mx-auto px-8">
                            <ul className="flex flex-col gap-10 cursor-pointer py-4">
                                {navItems.map((ele: navItem, idx: number) => (
